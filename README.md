...
В Go общий стиль кода задаёт **gofmt**. Отдельного файла конфигурации для него не бывает: либо код соответствует правилам языка, либо нет.

Что делает **`go fmt ./...`**: проходит по всем пакетам в дереве и переписывает `.go`-файлы в каноничном виде (отступы, пробелы вокруг операторов и т.д.). После нормального прогона повторный запуск уже ничего не меняет.
...
То же самое короче: **`make fmt`**.

### Линтер

Используется [golangci-lint](https://golangci-lint.run/) и файл **`.golangci.yml`**. Включены только смысловые проверки для такого проекта: `errcheck`, `govet`, `staticcheck`, `unused`, `ineffassign` и контроль **`gofmt`**, чтобы в репозиторий не уезжал «красивый в редакторе», но не отформатированный код.

Установка, если бинарника ещё нет: [релизы](https://github.com/golangci/golangci-lint/releases) под свою систему или:

```bash
go install github.com/golangci/golangci-lint/cmd/golangci-lint@latest
```

Положите **`$(go env GOPATH)/bin`** в **`PATH`**, иначе `make lint` и хук pre-commit не найдут программу.
...
Или **`make lint`**, **`make check`** (сначала `fmt`, потом `lint`).

### Pre-commit

**Pre-commit** — это не «ещё один линтер», а маленький запускатель: перед **`git commit`** он по очереди вызывает то, что перечислено в **`.pre-commit-config.yaml`**.

Один раз ставите сам инструмент (нужен Python, чаще всего хватает `pip` или `pipx`):

```bash
pip install pre-commit
```

В каталоге с клоном репозитория подключаете хуки к Git:
...
После этого при каждом **`git commit`** автоматически выполняется цепочка: хвостовые пробелы, перевод строки в конце файла, проверка YAML, ограничение на слишком большие файлы, затем **`go fmt`** по Go-файлам и **`golangci-lint`**. Если какой-то шаг падает, коммит **не создаётся**: смотрите вывод в терминале, исправляете, снова **`git add`** нужные файлы и повторяете **`git commit`**. Частый случай: хук сам поправил пробелы — тогда нужно заново добавить изменённые файлы в индекс.

Проверить то же самое **без** коммита (по всему дереву):

```bash
pre-commit run --all-files
```

Важно: в терминал копируйте **только** строки команд, без пояснений в той же строке после **`#`**. Иначе оболочка или `make` могут воспринять хвост как лишние аргументы (например, `make` попытается собрать цель с именем `#`, а `pre-commit` — разобрать русский текст как опции).

Пример правильного набора команд (каждая строка отдельно):
